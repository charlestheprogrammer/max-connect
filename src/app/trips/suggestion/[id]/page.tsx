...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  await connect()
  const trip: TripSuggestion | null = await SuggestedTrip.findById(id)
  ...
  return {
    title: `Voyage vers
            ${
              data?.find((station) => station.iata === trip?.to)?.name
            } du ${formatTripDate(
      trip?.route_from[0][0].heure_depart,
      false
    )} au ${formatTripDate(trip?.route_to[0][0].heure_depart, false)}`,
...
export default async function Page(props: {
  ...
  await connect()
  const trip: TripSuggestion | null = await SuggestedTrip.findById(params.id)
  ...
  return (
    <div className="px-4 pt-4">
      <div
        ...
        style={{
          backgroundImage: `url('/city/${trip.to.toLowerCase()}.jpg')`,
        }}
      >
        <div className="w-full h-full backdrop-blur-sm flex items-center justify-center flex-col bg-[#00000030] gap-2 relative p-4">
          ...
          <h3 className="text-gray-100 font-medium text-sm md:text-lg text-center">
            {formatTripDate(trip.route_from[0][0].heure_depart, false)} au{' '}
            {formatTripDate(trip?.route_to[0][0].heure_depart, false)} •{' '}
            {trip.route_from.length} options au départ • {trip.route_to.length}{' '}
            options au retour
          </h3>
          <Link href="/">
            ...
            <div className="flex flex-col gap-2 text-sm text-gray-600">
              <div className="flex justify-between">
                ...
                <span>
                  {
                    trip[
                      searchParams?.way === 'to' ? 'route_to' : 'route_from'
